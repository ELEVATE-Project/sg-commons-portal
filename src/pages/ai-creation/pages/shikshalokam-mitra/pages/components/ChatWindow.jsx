          ...
          return (
            ...
            >
              <div className={`div36 ${chat?.source === "user" ? "": "flex-column !items-start"} ${i === chatsToShow.length - 1 ? "pb-0" : ""}`}>
                ...
                {chat?.file_url && (
                  <div className="mb-4 w-full">
                    <button className="h-[2.1875rem] flex items-center justify-center gap-[0.5rem] rounded-md border border-[#572E91] px-3 bg-[#572E91] font-medium text-sm leading-none text-white w-full md:w-[12.5rem]" onClick={() => handleDownloadFile(chat?.file_url, fileName, chat?.file_url?.split(".").pop())}>Download</button>
                  </div>
                )}
              </div>
